...
int main(int argc, char *argv[]) {
    ...
    int taskid,                     /* this task's unique id */
            ...
            ndims,reorder,periods[2], dim_size[2],coords[2], rank; /* for cartesian topology */
    MPI_Status status;
...
/* First, find out my taskid and how many tasks are running */
    ...
    dim_size[0] = 0; /* rows */
    dim_size[1] = 0; /* columns */
    periods[0] = 0; /* rows non-periodic */
    periods[1] = 0; /* columns non-periodic */
    reorder = 1; /* allows processes reordered for efficiency */
    MPI_Dims_create(numtasks,ndims,dim_size); /* let MPI make the split */
    MPI_Cart_create(MPI_COMM_WORLD, ndims, dim_size, periods, reorder, &cartesianCommunicator);
    printf("Cartesian Grid created with dim_size %d x %d\n",dim_size[0],dim_size[1]);

    if (taskid == MASTER) {
        for (i = 0; i <= numworkers; i++) { //0--->o master
            MPI_Cart_coords(cartesianCommunicator, i, ndims, coords);
            printf("%d, (%d,%d)\n ", i, coords[0], coords[1]);
        }
        ...
        for (i = 0; i < dim_size[0]; i++) {
            for (j = 0; j < dim_size[1]; j++) {
                coords[0] = i;
                coords[1] = j;
                MPI_Cart_rank(cartesianCommunicator, coords, &rank);
